...
import de.doppelkool.itemforgegui.Main.MenuComponents.*;
 ...
 */
public class AddScalarOperationMenu extends ConfirmableMenu {
	...
	public AddScalarOperationMenu(PlayerMenuUtility playerMenuUtility) {
		super(playerMenuUtility, 35);
		Double currentValue = this.playerMenuUtility.getAttributeStorage().getModifierValues().get(AttributeModifier.Operation.ADD_SCALAR);
		this.currentValue = currentValue != null ? currentValue : 0.0;
	}

	@Override
	public String getMenuName() {
		return "Edit value | Add Scalar Operation";
	}
	...
	public void handleMenu(InventoryClickEvent e) {
		...
		if (super.handleConfirm(e.getSlot(),
			() -> this.playerMenuUtility.getAttributeStorage().getModifierValues().put(AttributeModifier.Operation.ADD_SCALAR, this.currentValue),
	...
	private ItemStack getInfoBook() {
		...
		ItemStackCreateHelper.modifyLore(infoCurrentValuesBook,
			...
			ChatColor.GRAY + "-> " + ChatColor.YELLOW + "Operation: Add Scalar Operation",
			ChatColor.GRAY + "-> " + ChatColor.YELLOW + "Value: " + this.currentValue // TODO + "%"
		);
	...
	private void roundUpAndUpdate(double change) {
		this.currentValue = Math.ceil((this.currentValue + change) * 10) / 10.0 / 100;
	}
